...
public class Orders : ControlingClass
{
    ...
    public string Addord(DateTime ordate, string shipname, string shipcity, string shiparea, string shipaddress, string member)
    {
        this.ordate = ordate; this.shipname = shipname;
        this.shipcity = shipcity; this.shiparea = shiparea;
        this.shipaddress = shipaddress; this.member = member;
    ...
    {
        string query = string.Format("select * From Order Where {0} like'%{1}%' ", FullName, Password);
        return Search(query);
    }

    public DataTable Searchord(string Field, string Value)
    {
        string query = string.Format("select * From Order Where {0} like'%{1}%' ", Field, Value);
        return Searchord(query);
    }

    public DataTable Searchord(string query)
    {
        try
        {
